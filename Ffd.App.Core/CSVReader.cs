...
{
    /// <summary>
    /// Like the CsvWriter class, all methods in the CsvParser class are static. There are four parser methods that return a 
    /// DataTable from either a string or text stream, and expecting a header line in the CSV source or not. Only one method 
    /// has a real body, all others just adjust their parameter signature. The implementation is simple: the method reads one 
    /// "row" of the CSV source at a time and populates a row in the DataTable. The real meat is the private class CsvStream 
    /// explained below. There is one utility method that returns an unused column name for a DataTable, in case there are no 
    /// headers in the CSV source or the headers are not unique.
    /// </summary>
    public class CSVReader
    {
        ...
        /// <summary>
        /// The CsvStream class does the actual work - read the CSV source in one character at a time and return meaningful 
        /// chunks of decoded data, namely data items and rows.
        /// </summary>
